...
- また、Cursor Settings > Agents にて **Auto-Approved Mode Transitions** に `agent->plan` を設定することを推奨します。これにより、重要タスク（🔴）の際にエージェントがプランモードへ自動的に切り替え、計画を提示してから実装に進むフローが有効になります。
...
- さらに、ファイル参照・差分適用・コマンド実行などのツール利用ポリシーを明文化し、常に安全な手順・権限でタスクを進めるワークフローを整備しています。
...
2. パスが存在する場合は、`ja/.cursor/rules/v5/`（日本語）または `en/.cursor/rules/v5/`（英語）フォルダをコピーして配置してください。
3. テスト方針ルールを有効にしたい場合は、同じフォルダに `ja/.cursor/rules/test-strategy/`（日本語）または `en/.cursor/rules/test-strategy/`（英語）をコピーしてください。
4. コミットメッセージ規約を有効にしたい場合は、同じフォルダに `ja/.cursor/rules/commit-message-format/`（日本語）または `en/.cursor/rules/commit-message-format/`（英語）をコピーしてください。
5. PR メッセージ規約を有効にしたい場合は、同じフォルダに `ja/.cursor/rules/pr-message-format/`（日本語）または `en/.cursor/rules/pr-message-format/`（英語）をコピーしてください。
