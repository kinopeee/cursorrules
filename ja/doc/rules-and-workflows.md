...
- **ルール（Rules）**:  
  `.cursor/rules/*/RULE.md` に配置する、モデルが常に参照する「振る舞い・フォーマット」の規約。
...
1. **ルールは「何を・どう書くか」を定義する**  
   ...
   - 例:  
     - `commit-message-format/RULE.md`: コミットメッセージの Prefix / サマリ / 箇条書き本文の書き方を定義。  
     - `pr-message-format/RULE.md`: PR タイトル / 本文の構造（概要 / 変更内容 / テスト内容など）を定義。
     - `test-strategy/RULE.md`: テスト観点表、Given/When/Then コメント、カバレッジ方針などを定義。
...
2. **ワークフローは「どう実行するか」だけを書く**  
   ...
   - 例:  
     - `commit-only.md`: コミットだけを行う手順（`git add -A` → `git commit -m "$MSG"`）と、  
       メッセージの中身は `commit-message-format/RULE.md` に従うことを明記。  
...
- `commit-only.md`
  ...
  - コミットメッセージの中身は `commit-message-format/RULE.md` を参照。
...
- `commit-push-pr.md`
  ...
  - コミットメッセージは `commit-message-format/RULE.md`、PR メッセージは `pr-message-format/RULE.md` に従うことを前提に、  
...
flowchart TB
    ...
    RLabel[ルール<br/>.cursor/rules/*/RULE.md]
...
flowchart TB
    ...
    RLabelT[ルール<br/>.cursor/rules/*/RULE.md]
...
> **注**: `test-strategy/RULE.md` は、テストコードの作成・更新時のみ適用されます。
