...
- 本ルールは、Conventional Commits をベースにしたコミットメッセージ規約（`commit-message-format/RULE.md`）と整合する形で、PR メッセージのフォーマットを定義します。
...
- コミットメッセージ規約（`commit-message-format/RULE.md`）と整合するように Prefix やサマリを決める（コミットと PR で意味的な不整合が出ないようにする）。
