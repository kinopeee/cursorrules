...
- **スタンドアロン化**: ワークスペース共通ルールへの依存を削除し、単独で使用可能に
- **ツール名の抽象化**: 具体的なツール名（`read_file`, `apply_patch` 等）から抽象的な操作種別（`読み/検索`, `差分適用`）に変更し、異なる環境でも適用可能に
- **タスク判断の明確化**: 「判断に迷った場合は 🟡標準タスク として扱う」を追加
- **ルール優先順位の簡素化**: 競合時は「より具体的・制限的な条項を優先」という原則を明記
- **セクション構成の改善**: 「タスク分類と推論深度」→「タスク分類と標準フロー」に変更し、実行フローを統合
